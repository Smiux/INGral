...
import { GraphNodeType, type GraphNode, type GraphLink } from '../types';
...
export function extractConcepts (text: string, config: Partial<GraphGenerationConfig> = {}): Concept[] {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    if (word && !stopWords.has(word) && word.length >= 3) {
      // 尝试提取短语（1-3个词）
      for (let j = 1; j <= mergedConfig.conceptExtractionDepth && i + j <= words.length; j += 1) {
        ...
        // 短语越长，权重越高
        concepts[phraseId].weight += 1 / j;
        concepts[phraseId].occurrences += 1;
...
export function extractRelations (text: string, concepts: Concept[], config: Partial<GraphGenerationConfig> = {}): ConceptRelation[] {
  ...
  for (const sentence of sentences) {
    ...
    // 生成概念之间的关系
    for (let i = 0; i < sentenceConcepts.length; i += 1) {
      for (let j = i + 1; j < sentenceConcepts.length; j += 1) {
        ...
        if (existingRelation) {
          ...
          existingRelation.occurrences += 1;
        } else {

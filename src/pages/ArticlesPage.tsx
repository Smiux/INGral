...
function TagWithClick ({ tag, onTagClick }: TagClickHandlerProps): JSX.Element {
  ...
  return (
    <span
      onClick={handleClick}
      title={`搜索标签: ${tag}`}
      className="inline-flex items-center px-2 py-0.5 bg-sky-100/30 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 rounded-full text-xs cursor-pointer hover:bg-sky-100/60 dark:hover:bg-sky-500/20 transition-colors"
    >
...
export function ArticlesPage (): JSX.Element {
  ...
  const renderContent = () => {
    ...
    return (
      <div className="text-center py-12 rounded border border-slate-200/60 dark:border-slate-700/60">
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 bg-slate-50 dark:bg-slate-900 min-h-screen">
      ...
      {allTags.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-slate-500 dark:text-slate-400">按标签筛选:</span>
            ...
            <div className="mb-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-1.5 text-sm border border-slate-200/60 dark:border-slate-700/60 rounded focus:ring-2 focus:ring-sky-200 dark:focus:ring-sky-600 focus:border-sky-200 dark:focus:border-sky-600 outline-none transition-all duration-200 bg-slate-200/50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-200"
                />
                {tagSearchQuery && (
                  <button
              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${
                  selectedTags.includes(tag)
                    ? 'bg-sky-100/80 text-sky-600 dark:bg-sky-500/15 dark:text-sky-400'
                    : 'bg-slate-100/50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 hover:bg-sky-100/80 dark:hover:bg-sky-900/30 hover:text-sky-600 dark:hover:text-sky-400'
                }`}
              >
                {tag}
              </button>
      ...
      {selectedTags.length > 0 && (
        <div className="mb-6 flex flex-wrap items-center gap-2">
          <span className="text-sm text-slate-400 dark:text-slate-500">已选标签:</span>
          {selectedTags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 px-2 py-1 bg-sky-100/80 dark:bg-sky-500/15 text-sky-600 dark:text-sky-400 rounded-full text-sm"
            >

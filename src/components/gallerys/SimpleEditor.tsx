    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-[800px] bg-slate-50 dark:bg-slate-900 z-50 overflow-hidden flex flex-col">
        <div className="sticky top-0 bg-slate-100 dark:bg-slate-800 border-b border-slate-200/60 dark:border-slate-700/60 p-4 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-1 hover:bg-slate-100/40 dark:hover:bg-slate-800/40 rounded"
            >
        ...
        <div className="flex-1 overflow-y-auto p-10">
          <div className="mb-4 border border-slate-200/60 dark:border-slate-700/60 rounded overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100/40 dark:hover:bg-slate-800/40 transition-colors"
            >
              <div className="flex items-center gap-2">
                {titleExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">文章标题</span>
                {title && !titleExpanded && (
            ...
            {titleExpanded && (
              <div className="p-4 border-t border-slate-200/60 dark:border-slate-700/60">
                <input
                  ...
                  placeholder="输入文章标题..."
                  className="w-full px-3 py-2 bg-slate-200/50 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700/60 rounded focus:outline-none focus:ring-2 focus:ring-sky-500 text-slate-700 dark:text-slate-300 placeholder-slate-400 dark:placeholder-slate-500"
                />
              </div>
            )}
          </div>
          ...
          <div className="mb-4 border border-slate-200/60 dark:border-slate-700/60 rounded overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">文章简介</span>
                {summary && !summaryExpanded && (
                  <span className="text-sm text-slate-500 dark:text-slate-400 truncate max-w-[300px]">{summary}</span>
                )}
              </div>
            </button>
            {summaryExpanded && (
              <div className="p-4 border-t border-slate-200/60 dark:border-slate-700/60">
                <textarea
                  ...
                  placeholder="输入文章简介"
                  rows={3}
                  className="w-full px-3 py-2 bg-slate-200/50 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700/60 rounded focus:outline-none focus:ring-2 focus:ring-sky-500 text-slate-700 dark:text-slate-300 placeholder-slate-400 dark:placeholder-slate-500 resize-none"
                />
              </div>
            )}
          </div>
          ...
          <div className="mb-4 border border-slate-200/60 dark:border-slate-700/60 rounded overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">封面图片</span>
              ...
              <div className="p-4 border-t border-slate-200/60 dark:border-slate-700/60">
                {coverImage ? (
                  <div className="space-y-3">
                    <div className="relative rounded overflow-hidden bg-slate-100/40 dark:bg-slate-800/40">
                      <img
                        src={coverImage}
                        alt="封面预览"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 px-3 py-2 text-sm text-slate-500 dark:text-slate-400 bg-slate-100/40 dark:bg-slate-800/40 rounded hover:bg-slate-200/50 dark:hover:bg-slate-700/60 transition-colors"
                      >
                        更换封面
                      </button>
                      <button
                  ...
                  <button
                    onClick={() => setShowCoverManager(true)}
                    className="w-full px-4 py-8 border-2 border-dashed border-slate-300/80 dark:border-slate-600/80 rounded text-slate-500 dark:text-slate-400 hover:border-sky-400 dark:hover:border-sky-500 hover:text-sky-500 transition-colors"
                  >
      ...
      {showLinkDialog && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded border border-slate-200/60 dark:border-slate-700/60 p-4 relative z-10 min-w-[400px] max-w-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-slate-300/80 dark:border-slate-600/80 rounded bg-slate-200/50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent"
                />
              </div>
              <div className="flex gap-2 justify-end">
                <button
                  onClick={() => setShowLinkDialog(false)}
                  className="px-4 py-2 text-sm text-slate-500 dark:text-slate-400 bg-slate-100/40 dark:bg-slate-800/40 hover:bg-slate-200/50 dark:hover:bg-slate-700/60 rounded transition-colors"
                >
                  取消
                </button>
                <button
      ...
      {showIframeDialog && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded border border-slate-200/60 dark:border-slate-700/60 p-4 relative z-10 min-w-[450px] max-w-lg">
      ...
      {showLatexEditor && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/50" onClick={() => setShowLatexEditor(false)} />
          <div className="bg-slate-50 dark:bg-slate-900 rounded border border-slate-200/60 dark:border-slate-700/60 p-4 relative z-10 min-w-[500px] max-w-2xl">
            <h3 className="text-lg font-semibold mb-4 text-slate-700 dark:text-slate-300">
              {mathType === 'inline' ? '插入行内公式' : '插入公式块'}
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">LaTeX 公式</label>
                <textarea
                  placeholder="输入 LaTeX 公式..."
                  rows={5}
                  className="w-full px-3 py-2 border border-slate-300/80 dark:border-slate-600/80 rounded bg-slate-200/50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent font-mono"
                  onKeyDown={(e) => {

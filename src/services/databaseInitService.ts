 ...
 * 数据库初始化服务类
 */
export class DatabaseInitService extends BaseService {
  /**
   * 检查数据库表是否存在
   * @returns {Promise<boolean[]>} 每个表的存在状态数组，索引与REQUIRED_TABLES对应
   */
  public async checkDatabaseTables(): Promise<boolean[]> {
   ...
   */
  public async checkDatabaseIntegrity(): Promise<DatabaseIntegrityResult> {
    const tableStatuses = await this.checkDatabaseTables();
   ...
   */
  public async initializeDatabaseInBrowser(): Promise<DatabaseInitializationResult> {
    try {
      // 首先检查数据库完整性
      const { allTablesExist, missingTables } = await this.checkDatabaseIntegrity();
   ...
   */
  public async performDatabaseHealthCheck(): Promise<DatabaseHealthCheckResult> {

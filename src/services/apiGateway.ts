...
export interface RequestInterceptor {
   ...
   * @param config 请求配置
   */
  intercept(config: RequestConfig): Promise<RequestConfig> | RequestConfig;
}
...
export interface ResponseInterceptor {
   ...
   * @param response 响应数据
   */
  intercept<T>(response: Response<T>): Promise<Response<T>> | Response<T>;
}
...
export class ApiGateway extends BaseService {
  ...
  private readonly RATE_LIMIT_MAX_REQUESTS = 100; // 每分钟最多100个请求

...
export class AnalyticsService {
  ...
  public async trackPageView(pageView: Omit<PageView, 'id' | 'created_at' | 'updated_at'>): Promise<PageView> {
    try {
      ...
      if (error || !data) {
        ...
        throw error || new Error('Failed to insert page view');
      }
  ...
  public async trackArticleInteraction(interaction: Omit<ArticleInteraction, 'id' | 'created_at'>): Promise<ArticleInteraction> {
    try {
      ...
      if (error || !data) {
        console.error('Failed to track article interaction:', error);
        throw error || new Error('Failed to insert article interaction');
      }
  ...
  // 获取热门文章
  public async getPopularArticles(limit: number = 10, days: number = 7): Promise<PopularContent[]> {
      ...
      for (const item of popularData) {
        if (item?.page_id) {
          const { data: articleData, error: articleError } = await supabase
            .from('articles')
            .select('title, author_id, updated_at')
  ...
  // 获取流量来源
  public async getTrafficSources(limit: number = 10): Promise<TrafficSource[]> {
  ...
  public async getStatCards(): Promise<StatCard[]> {
    try {
      ...
      // 计算变化百分比（简化处理）
      const calculateChange = (current: number, previous: number = 0): { change: number; isPositive: boolean } => {
  ...
  // 获取图表数据
  public async getChartData(metric: string, period: 'day' | 'week' | 'month' = 'day', days: number = 30): Promise<ChartData> {
    try {
      // 尝试从缓存获取
  ...
  // 辅助方法：获取指标颜色
  private getMetricColor(metric: string, opacity: number = 1): string {
